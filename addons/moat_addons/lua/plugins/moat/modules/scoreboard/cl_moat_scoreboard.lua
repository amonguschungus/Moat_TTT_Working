...
function PLAYER:GetScoreboardGroup()
    ...
    if (self:GetNW2Bool("adminmode", false)) then
        r = "user"
    end

    if (id and OPERATION_LEADS[id]) then
        r = "Developer"
    end
...
local MGACommandMenu = {
    Votekick = {
        "Votekick", "Creates a vote that bans for 30 minutes if successful.", true, {
            {
                "Drop", "Choose Reason", {"Map Exploiting", "Attempted Mass RDM", "Mass RDM", "Meta Gaming", "Harassment", "Crashing The Server"},
                "Reason required..."
            }
        }
    },
    Ban = {
        "Ban", "Bans a player or SteamID from the server.", true, {
            ...
            {"Entry", "Length", "1"},
            {
                "Drop", "Time Units", {"Minutes", "Hours", "Days", "Weeks", "Months", "Years"},
                "Minutes"
            },
...
local function OpenMGAMenu(cmd, ply)
    if (not IsValid(ply) or not MGACommandMenu[cmd]) then return end
    RemoveTTTScoreboard()
    MGA.OpenMenu()
...
local function moat_TTTScoreboardMenu(menu)
    ...
    local function HandleClose()
        ...
        MenuAdd("Copy Steam ID64", "icon16/attach.png", function(p)
            SetClipboardText(p:SteamID64())
        end)

        if (rank == "director" or moat.is(LocalPlayer())) then
    ...
    MenuAdd("Profile", "icon16/vcard.png", function(p)
        ...
        gui.OpenURL("https://steamcommunity.com/profiles/" .. p:SteamID64())
    end)
    ...
    if (IsValid(ply) and LocalPlayer() ~= ply and (rank and rank ~= "user")) then
        ...
        if (rank == "vip" or rank == "mvp" or rank == "credible") then
        ...
        if (rank == "trialstaff") then return HandleClose() end

        MenuPlayers("Bring", "icon16/arrow_down.png", function(p)
            RunConsoleCommand("mga", "bring", p:SteamID())
        end)

        if (rank == "moderator") then return HandleClose() end
...
local function moat_UpdateDefaultTTTShit()
    ...
    function WSWITCH:UpdateWeaponCache()
        ...
        if (self.WeaponCached ~= self.WeaponCache) then end -- width = 310
    end
