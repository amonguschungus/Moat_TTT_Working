...
if SERVER then
    ...
    function moatStartRaffle(ply)
        net.Start('moat_startRaffle')
        net.Broadcast()

        -- Plus one so it does it after the winner is announced
        timer.Simple(raffleTimer + 1, function()
            local plys = {}

            for k, v in pairs(player.GetAll()) do
                if moat.isdev(v) then continue end
                table.insert(plys, v)
            end
    ...
    concommand.Add('moat_raffle_start', function(ply, cmd, args, argStr)
        if not moat.isdev(ply) then
            ply:ChatPrint('You cannot start a raffle!')
    ...
    local raffleSong, raffleStation = "", nil
    ...
    net.Receive('moat_startRaffle', function(len)
        ...
        if (not playerAvatarRoulette) then
        ...
        timer.Create('moat_raffle_cyclePlayers', 0.3, 0, function()
            ...
            if (CurTime() >= endtime) then
                timer.Remove('moat_raffle_cyclePlayers')
