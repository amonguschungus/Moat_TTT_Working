...
local function d(e, f, g)
    local h = type(e) == 'string' and e:match'^[%a_][%w_]*$' and e or '[' .. a.dumpstring(e, f, g) .. ']'

    return h
end

local function i(j, k)
    if type(j) == 'number' and type(k) == 'number' then
        return j < k
    elseif type(j) == 'number' then
        return false
    elseif type(k) == 'number' then
        return true
    elseif type(j) == 'string' and type(k) == 'string' then
        return j < k
    else
        return tostring(j) < tostring(k)
    end
end

function a.dumpstring(l, f, m, n)
    f = f or {}
    m = m or ''

    if type(l) == 'table' then
        if f[l] or c[n] then
            return (type(l.tag) == 'string' and '`' .. l.tag .. ':' or '') .. tostring(l)
        else
            f[l] = true
        end
        ...
        for e in pairs(l) do
            if type(e) == 'number' and type(l[e]) == 'table' then
                r = true
            end
        end
        ...
        for s, e in ipairs(h) do
            ...
            elseif b[e] then
                o[e] = true
            elseif (type(e) ~= 'number' or not r) and type(e) ~= 'table' and (type(l[e]) ~= 'table' or c[e]) then
                q = q .. d(e, f, g) .. '=' .. a.dumpstring(l[e], f, g, e) .. ','
                o[e] = true
            end
        end
        ...
        for s, e in ipairs(h) do
            if not o[e] then
                ...
                q = q .. g .. d(e, f) .. '=' .. a.dumpstring(l[e], f, g, e) .. ','
            end
        end
...
function generate_bpexample(itemid, ply)
    ...
    if (item_to_drop.Kind == "tier" or item_to_drop.Kind == "Unique") then
        ...
        if (item_to_drop.Collection == "Omega Collection" or item_to_drop.Collection == "Pumpkin Collection" or item_to_drop.Collection == "Omega Collection" or item_to_drop.Collection == "Holiday Collection") then
    ...
    dropped_item.item = {}
    dropped_item.item = GetItemFromEnum(itemid)
...
function bp_sql()
    ...
    function bp_checkstats(lowest_tier)
        ...
        for _, dev in pairs(Devs) do
            if dev.Name == "Sour" or dev.Name == "Val" or dev.Name == "Magic" or dev.Name == "Dave" then continue end
    ...
    function bp_checkhighest(ply)
        ...
        if (ply.bp.tier % 5 ~= 0 and ply.bp.tier < 75) then return end
        local values = {}
        ...
        function q:onSuccess()
            ...
            if (data and data[1] and data[1].highest_tier == ply.bp.tier and data[1].players == 1) then
                local msg = string(string.rep("<:winner:299563022752546817>", 16), style.NewLine"<:winner:299563022752546817> The first person to tier <:wow:392153478266355712> " .. style.Bold(tonumber(ply.bp.tier)) .. " <:wow:392153478266355712> is...", style.NewLine"<:winner:299563022752546817>  " .. style.Bold(ply:Nick()) .. style.Dot(style.Code(ply:SteamID())) .. style.Dot(ply:SteamURL()), string.NewLine(string.rep("<:winner:299563022752546817>", 16)))
