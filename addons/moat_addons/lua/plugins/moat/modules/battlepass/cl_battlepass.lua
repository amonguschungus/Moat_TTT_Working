...
MOAT_BP = istable(MOAT_BP) and MOAT_BP or {
    tiers = {},
    current_tier = 0,
    xp = 0,
    Examples = {}
...
function make_randompanel(model, ITEM_BG)
    ...
    if (m_WClass) then
        if (not string.EndsWith(tostring(m_WClass.WorldModel), ".mdl")) then
            if (not IsValid(m_DPanelIcon.SIcon.Icon)) then
                m_DPanelIcon.SIcon:CreateIcon(n)
            end
    ...
    m_DPanelIcon.SIcon.PaintOver = function(s, w, h)
        if (not string.EndsWith(m_DPanelIcon.WModel, ".mdl")) then
            ...
            if (m_DPanelIcon.WModel:StartWith("https")) then
                cdn.DrawImage(m_DPanelIcon.WModel, 0, 0, w, h, {
                    r = 255,
                    g = 255,
                    b = 255,
                    a = 255
...
function make_battlepass()
    ...
    M_BP.Paint = function(s, w, h)
        local tl = util.TimeRemaining(1617303600, os.time())
        local left = string(tl.days, " day" .. (tl.days == 1 and "" or "s") .. ", ", tl.hours, " hr" .. (tl.hours == 1 and "" or "s") .. ", ", tl.minutes, " min" .. (tl.minutes == 1 and "" or "s") .. "", tl.seconds, " sec" .. (tl.seconds == 1 and "" or "s"))
    ...
    timer.Create("BP.HoveredExampleRateLimit", 5, 0, function()
        if not IsValid(tiers) then
            timer.Remove("BP.HoveredExampleRateLimit")
    ...
    for tier, v in pairs(table.Reverse(MOAT_BP.tiers)) do
        ...
        if v.model == "tier" or v.model == "crates" then
            timer.Create(tostring(s), 3, 0, function()
                ...
                if not IsValid(BPMODEL_PANELS[tier]) then
                    timer.Remove(tostring(s))
    ...
    function BP_HOVER.Paint(s, w, h)
        ...
        if (ITEM_HOVERED and ITEM_HOVERED.c) then
            ...
            if (not ITEM_NAME_FULL) then
                ITEM_NAME_FULL = "Error with Item Name"
            end
            ...
            surface_SetDrawColor(rarity_gradient[ITEM_HOVERED.item.Rarity].r, rarity_gradient[ITEM_HOVERED.item.Rarity].g, rarity_gradient[ITEM_HOVERED.item.Rarity].b, rarity_gradient[ITEM_HOVERED.item.Rarity].a or color_white)
            local grad_x = 1
            ...
            surface_SetDrawColor(rarity_names[ITEM_HOVERED.item.Rarity][2].r, rarity_names[ITEM_HOVERED.item.Rarity][2].g, rarity_names[ITEM_HOVERED.item.Rarity][2].b, 100 or color_white)
    ...
    BP_HOVER.Think = function(s)
        ...
        if (ITEM_HOVERED and ITEM_HOVERED.c) then
            ...
            if (not s.savewide) then
                s.savewide = s:GetWide()
            end
...
function m_CreateBattlePanel(pnl_x, pnl_y, pnl_w, pnl_h)
    ...
    M_BATTLE_PNL.Paint = function(s, w, h)
        ...
        draw.SimpleText("TTT", "moat_GambleTitle", 45, 1, Color(50, 50, 50))
        draw.SimpleText("Moat", "moat_GambleTitle", 4, 0, Color(0, 198, 255))
        draw.SimpleText("TTT", "moat_GambleTitle", 45, 0, Color(255, 255, 255))
    ...
    local MOAT_BP_CATS = {
        {"Summer Climb", Color(255, 0, 50)},
        {"About", Color(150, 0, 255)}
    }
    ...
    for i = 1, #MOAT_BP_CATS do
        ...
        MOAT_BP_CAT_BTN.OnCursorEntered = function()
            if (GetConVar("moat_ui_sounds"):GetInt() > 0) then
...
include('plugins/moat/modules/battlepass/sh_battlepass.lua')