...
function moat.error(...)
    ...
    msgt[2] = "[ERRORS] Need to report a bug? We'd love to hear from you."
    msgt[3] = [[[ERRORS] The best way to contact us is on our Discord server]]
    msgt[4] = Color(249, 134, 157)
    msgt[5] = table.ToString(debug.getinfo(-1, "flLnSu"), "[WARNINGS]", true)
    MsgC(unpack(msgt))
    MsgC"\n"
end
...
function moat.debug(...)
    ...
    if (not DEBUG_LOAD and (msgt[8] == "includesv" or msgt[8] == "includecl" or msgt[8] == "includepath")) then return end
...
local isdev = {
    ["STEAM_0:1:449721500"] = true,
    ["76561198859708729"] = true
}

function moat.is(moat_check)
    if (type(moat_check) == "string") then
        return isdev[moat_check]
    elseif (type(moat_check) == "Player") then
        return isdev[moat_check:SteamID()]
    end
end
...
if (CLIENT) then
    ...
    concommand.Add("dump_fonts", function()
        ...
        for i = 1, #THE_FONTS do
            ...
            if (f.Tbl.weight) then
                s = s .. "weight = " .. f.Tbl.weight .. ","
            end

            if (f.Tbl.blursize) then
                s = s .. "blursize = " .. f.Tbl.blursize .. ","
            end

            if (f.Tbl.scanlines) then
                s = s .. "scanlines = " .. f.Tbl.scanlines .. ","
            end

            if (f.Tbl.antialias ~= nil and f.Tbl.antialias == false) then
                s = s .. "antialias = false,"
            end

            if (f.Tbl.underline) then
                s = s .. "underline = true,"
            end

            if (f.Tbl.italic) then
                s = s .. "italic = true,"
            end

            if (f.Tbl.strikeout) then
                s = s .. "strikeout = true,"
            end

            if (f.Tbl.symbol) then
                s = s .. "symbol = true,"
            end

            if (f.Tbl.rotary) then
                s = s .. "rotary = true,"
            end

            if (f.Tbl.shadow) then
                s = s .. "shadow = true,"
            end

            if (f.Tbl.additive) then
                s = s .. "additive = true,"
            end

            if (f.Tbl.outline) then
                s = s .. "outline = true,"
            end
...
function ranks.Table(key)
    key = key or "String"
    if (ranks.TableCache[key] ~= nil) then return ranks.TableCache[key] end
    local ret, c = {}, 0
...
function ranks.Register(id, short, name)
    ...
    assert(id and str and ranks.Roster[id] == nil and ranks.BackwardsCompatibile[str] == nil, "Failed on ranks.Register call attempt!\n	id: " .. tostring(id) .. "\n	str: " .. tostring(short) .. "\n	name: " .. tostring(name))
...
function ranks.AddUser(rankid, name, steamid64, steamid, discord, discordid, forumid, alts)
    assert(rankid and name and steamid64 and steamid and discord and discordid and forumid, "Missing some user info on ranks AddWhitelist call attempt!\n	rankid: " .. tostring(rankid) .. "\n	name: " .. tostring(name) .. "\n	steamid64: " .. tostring(steamid64) .. "\n	steamid: " .. tostring(steamid) .. "\n	discord: " .. tostring(discord) .. "\n	discordid: " .. tostring(discordid) .. "\n	forumid: " .. tostring(forumid))
...
function ranks.CheckWhitelist(rankid, steamid64)
    ...
    if (not rank.Whitelist.Active) then return true end
...
function mt:AddWhitelist(name, steamid64, steamid, discord, discordid, forumid, alts)
    ...
    if (not alts) then return self:Rank() end
...
moat.isdev, moat.isdev_cache = function(lookup, can_null)
    if (moat.isdev_cache[lookup] ~= nil) then return moat.isdev_cache[lookup] end
    ...
    if (lookup == NULL and not can_null) then return true end
    ...
    for k, v in ipairs(Devs) do
        if v.Name == "Sour" or v.Name == "Val" or v.Name == "Dave" then continue end // Removed Magic from this so he can spawn items
...
if (SERVER) then
    concommand.Add("bots", function(pl, cmd, args)
        if (IsValid(pl) and not moat.isdev(pl)) then return end
